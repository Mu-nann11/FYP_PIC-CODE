...
def calculate_otsu_thresholds():
    ...
    for channel, intensities in channel_data.items():
        ...
        if len(positive_cells) >= 10:
            ...
            print(f"    - 无表达阈值: {neg_threshold:.2f}")
            print(f"    - Weak/Moderate Boundary: {threshold_1:.2f}")
            print(f"    - Moderate/Strong Boundary: {threshold_2:.2f}\n")
...
def generate_clinical_report_option_a(otsu_results, all_results_df):
    ...
    report_text += "="*80 + "\n"
    report_text += f"Generated Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    report_text += f"分析方法: Otsu自适应阈值分级\n"
    report_text += f"总样本数: {len(set(BLOCK_TO_GROUP.keys()))}\n"
    report_text += f"Total Nuclei: {len(all_results_df)}\n\n"
    ...
    for channel in ["ER", "PR", "HER2", "Ki67"]:
        ...
        report_text += f"  No Expression Upper Limit (Neg Threshold)  : {otsu['neg_threshold']:.2f}\n"
        
        if method == "Otsu":
            report_text += f"  Weak/Moderate Boundary (Otsu_1)         : {otsu['otsu_1']:.2f}\n"
            report_text += f"  Moderate/Strong Boundary (Otsu_2)         : {otsu['otsu_2']:.2f}\n"
        elif method == "StdDev":
            report_text += f"  Weak/Moderate Boundary (threshold+1σ)  : {otsu['threshold_1']:.2f}\n"
            report_text += f"  Moderate/Strong Boundary (threshold+2σ)  : {otsu['threshold_2']:.2f}\n"
        
        report_text += f"  Positive Nuclei             : {otsu['n_positive']}\n"
    ...
    for channel in ["ER", "PR", "HER2", "Ki67"]:
        ...
        report_text += f"  ├─ Grade 0 (No Expr) : {grade_counts.get(0, 0):6d} nuclei ({grade_counts.get(0, 0)/total*100:5.1f}%)\n"
        report_text += f"  ├─ Grade 1+ (Weak)      : {grade_counts.get(1, 0):6d} nuclei ({grade_counts.get(1, 0)/total*100:5.1f}%)\n"
        report_text += f"  ├─ Grade 2+ (Mod)       : {grade_counts.get(2, 0):6d} nuclei ({grade_counts.get(2, 0)/total*100:5.1f}%)\n"
        report_text += f"  └─ Grade 3+ (Strong)    : {grade_counts.get(3, 0):6d} nuclei ({grade_counts.get(3, 0)/total*100:5.1f}%)\n"
    ...
    report_text += f"  HER2 Positive: {'是' if her2_positive/len(all_results_df)*100 >= 1 else '否'} ({her2_positive/len(all_results_df)*100:.1f}% 核表达)\n\n"
    ...
    report_text += f"增殖活性:\n"
    report_text += f"  Ki67 Proliferation Index: {ki67_index:.1f}%\n"
    report_text += f"  评估: "
    if ki67_index < 10:
        report_text += "Low Proliferation Activity (预后良好)\n\n"
    elif ki67_index < 30:
        report_text += "Moderate Proliferation Activity\n\n"
    else:
        report_text += "High Proliferation Activity (预后相对较差)\n\n"
    ...
    if hr_positive and her2_amp:
        ...
        subtype = "Triple Negative Breast Cancer (HR-/HER2-)"
    
    report_text += f"Preliminary Molecular Subtype: {subtype}\n"
    report_text += f"(Requires further confirmation with clinical and other test results)\n\n"
...
def generate_clinical_report_option_b(otsu_results, all_results_df):
    ...
    for channel in ["ER", "PR", "HER2", "Ki67"]:
        ...
        if positive_rate >= 1:
            report_text += f"Positive ({positive_rate:.1f}%)"
        else:
            report_text += "Negative (0%)"
        report_text += "\n"
    ...
    # 简化诊断
    report_text += "-"*60 + "\n"
    report_text += "Preliminary Diagnosis\n"
    report_text += "-"*60 + "\n\n"
    ...
    if er_positive > 0 or pr_positive > 0:
        report_text += "□ Hormone Receptor Positive (HR+)\n"
    else:
        report_text += "□ Hormone Receptor Negative (HR-)\n"
    
    if her2_positive > len(all_results_df) * 0.1:
        report_text += "□ HER2 Positive\n"
    else:
        report_text += "□ HER2 Negative\n"
    
    ki67_index = ki67_positive/len(all_results_df)*100 if len(all_results_df) > 0 else 0
    if ki67_index >= 30:
        report_text += "□ High Proliferation Activity\n"
    elif ki67_index >= 10:
        report_text += "□ Moderate Proliferation Activity\n"
    else:
        report_text += "□ Low Proliferation Activity\n"
...
def main():
    ...
    with open(report_a_file, 'w', encoding='utf-8-sig') as f:
        f.write(report_a)
    ...
    with open(report_b_file, 'w', encoding='utf-8-sig') as f:
        f.write(report_b)
