"""
批量运行 TMAe 所有 Block 的 CPSAM 质-核分割。
用法：python batch_tmae.py
"""
import sys
...
OUTPUT_DIR = r"d:\Try_munan\FYP_LAST\results\cpsam_segmentation"

blocks = sorted([d.name for d in STITCHED.iterdir() if d.is_dir()])
print(f"发现 {len(blocks)} 个 TMAe Block: {blocks}")
print()
...
for block in blocks:
    ...
    if (out_block / f"{block}_features.csv").exists():
        print(f"[{block}] 已完成，跳过\n")
        results[block] = 0
        continue

    print("=" * 60)
    print(f"处理 Block: {block}")
    print("=" * 60)
